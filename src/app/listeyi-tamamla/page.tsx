...
export default function ListeyiTamamlaPage() {
  ...
  const handleShare = () => {
    if (!currentQ) return;
    // URL güncellendi: futbol-trivia.vercel.app -> futboltrivia.com.tr
      ...
      <div className="text-center mb-4 relative z-10 font-sans">
        ...
        <div className="flex justify-between items-center mb-4 px-6 font-sans">
            <div className="text-left font-sans font-sans">
                <span className="font-bebas text-xs text-slate-500 block uppercase tracking-widest font-sans font-sans">Bulunan</span>
                <span className="font-bebas text-4xl text-white font-sans font-sans">{foundItems.length}<span className="text-slate-700 text-2xl font-sans font-sans">/{currentQ.targets.length}</span></span>
            </div>
            <div className="relative w-24 h-24 flex items-center justify-center font-sans font-sans">
                {bonusAnim && <div className="absolute -right-2 -top-2 text-green-500 font-bold text-xl animate-bounce-up z-20 font-sans font-sans">+5</div>}
                <svg className="transform -rotate-90 w-full h-full absolute inset-0 font-sans font-sans">
                ...
                <span className="font-bebas text-3xl text-white font-sans font-sans">{timeLeft}</span>
            </div>
        </div>
      </div>

      <div className="flex-grow overflow-y-auto mb-4 px-1 custom-scrollbar relative z-10 font-sans">
        <div className="flex flex-wrap gap-2 mb-4 font-sans font-sans">
          {foundItems.map((p, i) => (
            <div key={i} className="bg-green-600/20 border border-green-500/40 px-3 py-1.5 rounded-lg animate-in zoom-in duration-200 font-sans font-sans">
              <span className="text-xs font-semibold text-green-400 font-sans font-sans">{formatName(p)}</span>
            </div>
          ))}
          {showAll && currentQ.targets.filter((t: string) => !foundItems.some(f => normalizeText(f) === normalizeText(t))).map((t: string, i: number) => (
            <div key={i} className="bg-sky-900/30 border border-sky-400/30 px-3 py-1.5 rounded-lg font-sans font-sans">
              <span className="text-xs font-semibold text-sky-400 font-sans font-sans">{formatName(t)}</span>
            </div>
          ))}
        </div>
      ...
      {showStatsPopup && (
        <div className="absolute inset-0 bg-slate-950/90 z-[110] flex items-center justify-center p-6 animate-in fade-in duration-500 font-sans font-sans">
          <div className="w-full bg-slate-900 border-2 border-slate-800 rounded-3xl p-6 shadow-2xl relative text-white text-center font-sans">
            ...
            <h3 className="font-bold text-lg mb-6 italic font-sans font-sans">İstatistikler</h3>
            
            <div className="grid grid-cols-2 gap-4 mb-8 font-sans font-sans">
              <div className="bg-slate-950/50 p-4 rounded-2xl border border-slate-800 font-sans font-sans">
                <div className="text-2xl font-bold font-sans font-sans">{stats.totalGames}</div>
                <div className="text-[10px] text-slate-500 font-medium mt-1 font-sans font-sans">Toplam oyun</div>
              </div>
              <div className="bg-slate-950/50 p-4 rounded-2xl border border-slate-800 font-sans font-sans">
                <div className="text-2xl font-bold text-green-500 font-sans font-sans">{stats.wins}</div>
                <div className="text-[10px] text-slate-500 font-medium mt-1 font-sans font-sans">Tamamlanan liste</div>
              </div>
              <div className="bg-slate-950/50 p-4 rounded-2xl border border-slate-800 font-sans font-sans">
                <div className="text-2xl font-bold text-red-500 font-sans font-sans">{stats.bestScore}</div>
                <div className="text-[10px] text-slate-500 font-medium mt-1 font-sans font-sans">Doğru cevap rekoru</div>
              </div>
              <div className="bg-slate-950/50 p-4 rounded-2xl border border-slate-800 font-sans font-sans">
                <div className="text-2xl font-bold text-sky-500 font-sans font-sans">%{stats.highestPercentage}</div>
                <div className="text-[10px] text-slate-500 font-medium mt-1 font-sans font-sans">En iyi yüzde</div>
              </div>
            </div>

            <div className="flex items-center justify-between p-3 bg-slate-950/50 rounded-2xl border border-slate-800 mb-6 text-left font-sans font-sans">
              <div>
                <p className="text-slate-500 text-[10px] font-bold font-sans font-sans">Sıradaki soru</p>
                <p className="font-bold text-lg text-white mt-1 leading-none font-sans font-sans">{nextGameTime}</p>
              </div>
              {!isWin && !showAll && (
                <button onClick={() => { setShowAll(true); setShowStatsPopup(false); }} className="px-4 py-2 bg-slate-800 text-white rounded-xl text-[11px] font-bold font-sans font-sans">Cevapları Gör</button>
              )}
            </div>

            <button onClick={handleShare} className="w-full bg-white text-black py-4 rounded-2xl font-bold text-sm active:scale-95 transition-transform font-sans font-sans">
        ...
        {!isGameOver ? (
          <div className="relative font-sans font-sans">
            {suggestions.length > 0 && (
              <div className="absolute bottom-full w-full mb-2 bg-slate-800 border border-slate-700 rounded-xl overflow-hidden z-50 shadow-2xl font-sans font-sans">
                {suggestions.map((s, i) => (
                  <button key={i} onClick={() => handleGuess(s)} className={`w-full p-4 text-left border-b border-slate-700 last:border-0 text-sm font-semibold text-white font-sans font-sans ${selectedIndex === i ? "bg-red-600" : "hover:bg-red-600"}`}>{formatName(s)}</button>
                ))}
              </div>
            )}
            <input 
              ...
              className="w-full p-4 bg-slate-900 rounded-2xl border-2 border-slate-800 focus:border-red-600 outline-none font-bold text-base font-sans font-sans text-white font-sans" 
            />
          </div>
        ) : (
          <div className="p-4 bg-slate-900 rounded-2xl border-2 border-slate-800 flex gap-2 font-sans font-sans">
            <button onClick={() => setShowStatsPopup(true)} className="flex-grow py-4 bg-slate-800 text-white rounded-xl font-bold text-sm tracking-wide font-sans font-sans font-sans">İSTATİSTİKLER</button>
            <button onClick={handleShare} className="px-8 py-4 bg-white text-black rounded-xl font-bold text-sm transition-all active:scale-95 font-sans font-sans font-sans">
