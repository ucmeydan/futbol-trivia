...
export default function ListeyiTamamlaPage() {
  ...
  const finishGame = (won: boolean) => {
    ...
    setIsActive(false);
    setIsWin(won);
    saveStats(won, foundItems.length, currentQ.targets.length);
  ...
  const handleGuess = (guess: string) => {
    ...
    const isCorrect = currentQ.targets.some(t => normalizeText(t) === normalizedGuess);

    if (isCorrect) {
      const actualName = currentQ.targets.find(t => normalizeText(t) === normalizedGuess)!;
    ...
    <div className="max-w-md mx-auto h-screen flex flex-col p-4 text-white bg-slate-950 overflow-hidden font-sans relative">
      ...
      <div className="text-center mb-4 relative z-10">
        <div className="flex items-center justify-center gap-4 mb-2">
          <button onClick={() => { setCurrentIndex(currentIndex - 1); setFoundItems([]); setWrongGuesses([]); setTimeLeft(90); setIsGameOver(false); }} disabled={currentIndex === 0 || isActive} className="text-slate-700 hover:text-red-600 disabled:opacity-0 font-bebas text-xl transition-colors">‹</button>
          <div className="font-bebas text-sm text-red-600 tracking-tighter uppercase">#{currentQ.id}</div>
          <button onClick={() => { setCurrentIndex(currentIndex + 1); setFoundItems([]); setWrongGuesses([]); setTimeLeft(90); setIsGameOver(false); }} disabled={currentIndex === gameQuestions.length - 1 || isActive} className="text-slate-700 hover:text-red-600 disabled:opacity-0 font-bebas text-xl transition-colors">›</button>
        </div>
        ...
        <div className="flex justify-between items-center mb-4 px-6">
            ...
            <div className="relative w-24 h-24 flex items-center justify-center">
                ...
                <span className="font-bebas text-3xl text-white">{timeLeft}</span>
            </div>
        </div>
      </div>
      ...
      <div className="flex-grow overflow-y-auto mb-4 px-1 custom-scrollbar relative z-10">
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          {showAll && currentQ.targets.filter(t => !foundItems.some(f => normalizeText(f) === normalizeText(t))).map((t, i) => (
      ...
      {showStatsPopup && (
        <div className="absolute inset-0 bg-slate-950/90 z-[110] flex items-center justify-center p-6 animate-in fade-in duration-500">
          <div className="w-full bg-slate-900 border-2 border-slate-800 rounded-3xl p-6 shadow-2xl font-sans relative text-white text-center">
            ...
            <div className="grid grid-cols-2 gap-4 mb-8 font-sans">
              <div className="bg-slate-950/50 p-4 rounded-2xl border border-slate-800">
                ...
                <div className="text-[10px] text-slate-500 font-medium mt-1">Oynanan oyun</div>
              </div>
              <div className="bg-slate-950/50 p-4 rounded-2xl border border-slate-800">
                <div className="text-2xl font-bold text-green-500">{stats.wins}</div>
                <div className="text-[10px] text-slate-500 font-medium mt-1">Kazanılan oyun</div>
              </div>
              <div className="bg-slate-950/50 p-4 rounded-2xl border border-slate-800">
                <div className="text-2xl font-bold text-red-500">{stats.bestScore}</div>
                <div className="text-[10px] text-slate-500 font-medium mt-1">En çok doğru</div>
              </div>
              <div className="bg-slate-950/50 p-4 rounded-2xl border border-slate-800">
                <div className="text-2xl font-bold text-sky-500">%{stats.highestPercentage}</div>
                <div className="text-[10px] text-slate-500 font-medium mt-1">En yüksek yüzde</div>
              </div>
            </div>

            <div className="flex items-center justify-between p-3 bg-slate-950/50 rounded-2xl border border-slate-800 mb-6 text-left">
              <div>
                <p className="text-slate-500 text-[10px] font-bold uppercase tracking-tight">Sonraki Soru</p>
          ...
          <div className="relative">
            {suggestions.length > 0 && (
              <div className="absolute bottom-full w-full mb-2 bg-slate-800 border border-slate-700 rounded-xl overflow-hidden z-50 shadow-2xl">
                {suggestions.map((s, i) => (
                  <button key={i} onClick={() => handleGuess(s)} className={`w-full p-4 text-left border-b border-slate-700 last:border-0 text-sm font-semibold ${selectedIndex === i ? "bg-red-600" : "hover:bg-red-600"}`}>{formatName(s)}</button>
                ))}
              </div>
            )}
            <input 
              ...
              className="w-full p-4 bg-slate-900 rounded-2xl border-2 border-slate-800 focus:border-red-600 outline-none font-bold text-base" 
            />
          </div>
        ) : (
          <div className="p-4 bg-slate-900 rounded-2xl border-2 border-slate-800 flex gap-2">
            <button onClick={() => setShowStatsPopup(true)} className="flex-grow py-4 bg-slate-800 text-white rounded-xl font-bold text-sm tracking-wide">İSTATİSTİKLER</button>
            <button onClick={handleShare} className="px-8 py-4 bg-white text-black rounded-xl font-bold text-sm transition-all active:scale-95">
               {copySuccess ? "KOPYALANDI" : "PAYLAŞ"}
            </button>
          </div>
        )}
