    ...
    return (
      <div className="max-w-md mx-auto min-h-screen flex flex-col items-center justify-start pt-12 p-6 text-white bg-slate-950 font-sans text-center overflow-y-auto pb-10">
        <div className="mb-2 flex items-center justify-center gap-6">
          <button onClick={() => checkAndLoadQuestion(currentIndex - 1, gameQuestions)} disabled={currentIndex === 0} className="text-slate-700 hover:text-red-600 disabled:opacity-0 font-bebas text-4xl transition-colors">‹</button>
          <span className="text-red-600 font-bebas text-3xl leading-none">#{currentQ.id}</span>
          <button onClick={() => checkAndLoadQuestion(currentIndex + 1, gameQuestions)} disabled={currentIndex === gameQuestions.length - 1} className="text-slate-700 hover:text-red-600 disabled:opacity-0 font-bebas text-4xl transition-colors">›</button>
        </div>
        ...
        <div className="flex flex-col gap-5 w-full max-w-sm">
          <button onClick={() => setDifficulty('easy')} className="group flex flex-col items-center p-6 bg-green-600/5 border-2 border-green-500/10 hover:border-green-500 hover:bg-green-600/10 rounded-3xl transition-all duration-300 hover:scale-[1.02] active:scale-95">
          ...
          <button onClick={() => setDifficulty('hard')} className="group flex flex-col items-center p-6 bg-red-600/5 border-2 border-red-500/10 hover:border-red-500 hover:bg-red-600/10 rounded-3xl transition-all duration-300 hover:scale-[1.02] active:scale-95">
        ...
        {/* SEO içerik bloğu — zorluk seçim ekranı, koşulsuz */}
        <section className="mt-16 pt-8 border-t border-slate-900 w-full text-left">
          ...
          <div className="space-y-3 text-slate-700 text-xs leading-relaxed font-light">
            <p>
              ...
              kategoriye ait 10 ismi bulmak hedeflenir; doğru sıralama aranmaz, yalnızca
              listenin tamamını tamamlamak yeterlidir.
            </p>
            <p>
              Oyunun iki farklı zorluk modu bulunur. Kolay modda her ismin kaç harften
              oluştuğunu gösteren ipuçları ekranda görünür; bu sayede tahminlerini
              daraltmak daha kolay olur. Zor modda ise herhangi bir ipucu sunulmaz ve
              tamamen hafızana güvenmen gerekir.
            </p>
            <p>
              Her oyunda toplam 3 yanlış tahmin hakkın vardır. Bir ismi doğru
              bulduğunda liste üzerinde yeşil olarak işaretlenir. Üç yanlış
              tamamlandığında oyun sona erer ve kalan isimler gösterilir.
            </p>
            <p>
              Sorular Süper Lig tarihini, milli takım kadrosunu, Avrupa kupalarındaki
              Türk kulüplerini, belirli sezonlara ait istatistikleri ve özel kategorileri
              kapsar. Her gün yeni bir soru gelir ve arşiv navigasyonuyla geçmiş sorulara
              da ulaşabilirsin.
            </p>
          </div>
        </section>
    ...
    <div className="max-w-md mx-auto min-h-screen flex flex-col p-3 text-white bg-slate-950 font-sans relative overflow-y-auto">
      ...
      <div className="flex-grow mb-3 px-1 relative z-10 space-y-1 font-sans">
        {currentQ.targets.map((name: string, i: number) => {
          ...
          return (
            <div key={i} className={`relative flex items-center justify-center h-9 px-4 rounded-xl border transition-all duration-500 overflow-hidden ${isFound ? "bg-green-600/10 border-green-500/40" : (showAll ? "bg-red-600/10 border-red-500/40" : "bg-slate-900/40 border-slate-900")}`}>
              {isJustFound && (
      ...
      <div className="mt-auto relative z-10 font-sans pb-4">
        {!isGameOver ? (
      ...
      <section className="mt-6 pt-6 border-t border-slate-900 pb-10 font-sans">
        <h2 className="text-slate-600 text-xs tracking-[0.2em] uppercase mb-4">Top 10 hakkında</h2>
        <div className="space-y-3 text-slate-700 text-xs leading-relaxed font-light">
          <p>
            Top 10, Süper Lig ve Türk futboluna ait istatistik listelerini tamamlamaya
          ...
          <p>
            Oyunun iki farklı zorluk modu bulunur. Kolay modda her ismin kaç harften
            oluştuğunu gösteren ipuçları ekranda görünür. Zor modda ise herhangi bir
            ipucu sunulmaz ve tamamen hafızana güvenmen gerekir. Her iki modda da
            toplam 3 yanlış tahmin hakkın vardır.
          </p>
          <p>
            Sorular Süper Lig tarihini, belirli sezonlardaki gol krallıklarını, Türkiye
            Kupası şampiyonlarını, milli takım kadrosunu ve Avrupa kupalarındaki Türk
            kulüplerinin performanslarını kapsar. Belirli bir milliyetten en çok maça
            çıkan futbolcular, tarihin unutulmaz transferleri veya son sezonun en iyi
            oyuncuları gibi özgün kategorilerle karşılaşabilirsin.
          </p>
          <p>
            Bir ismi doğru bulduğunda liste üzerinde yeşil olarak işaretlenir ve o isim
            bir daha gösterilmez. Üç yanlış hakkın dolduğunda oyun sona erer ve kalan
            isimler ekranda görünür hale gelir. Tüm 10 ismi bulan oyuncu listeyi
            tamamlamış sayılır.
          </p>
          <p>
            İstatistik sayfasında toplam oyun sayını, kazanma yüzdeni ve kaç doğru
            cevap bulduğunu gösteren dağılım grafiğini görebilirsin. Arşiv
            navigasyonuyla geçmiş soruları tekrar oynayabilirsin. Her gün yeni soru
            gelir, her gün yeni bir liste seni bekler.
          </p>
        </div>
      </section>
      ...
      <style jsx>{`
        ...
        @keyframes shimmer { 0% { transform: translateX(-100%) skewX(-20deg); opacity: 0; } 50% { opacity: 0.8; } 100% { transform: translateX(100%) skewX(-20deg); opacity: 0; } }
        .animate-shimmer { animation: shimmer 0.6s ease-out forwards; }
      `}</style>
    </div>
  );
}